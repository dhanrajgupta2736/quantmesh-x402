...
2. **FinBERT Sentiment ($0.002 USDC / 2,000 µUSDC)** — Single-agent HuggingFace FinBERT Financial NLP sentiment analysis.
...
|---|---|---|---|
| **Frontend UI** | Cloudflare Pages (24/7 Edge CDN) | [https://qm.dhanrajgupta.xyz](https://qm.dhanrajgupta.xyz) | 🟢 Live |
| **API Gateway** | AWS EC2 (Hono + HTTPS) | `https://api.dhanrajgupta.xyz/api/v1/orchestrate` | 🟢 Live |
...
│   ├── src/app/page.tsx         # Terminal UI with sub-agent radar & score gauge
...
│   ├── src/index.ts             # Pre-execution pipeline, CORS & payment verification
...
Navigate to [http://localhost:3000](http://localhost:3000) to run the terminal locally.
...
- **Binance Live Market Feed**: 100% real-time unthrottled orderbook and OHLC candle data across 9 assets.
